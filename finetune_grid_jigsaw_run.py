...
saving_logs_path = '/mnt/DADOS_CERGY_1/bruno/master_project/fine_jigsaw_grid_logs'
...
if args['dataset']=='F3_netherlands':
    ...
    saved_models_segmentation_path = '/mnt/DADOS_CERGY_1/bruno/saved_models_f3/fine_jigsaw_grid'
    load_path = '/mnt/DADOS_CERGY_1/bruno/saved_models_f3/jigsaw_grid_search'
    grid = get_grid_search_finetune_jigsaw_f3()
    
elif args['dataset']=='Parihaka_NZPM': 
    n_few_shot = int(np.round(args['train_set_size_parih'] * args['p_few_shot'],0))
    saved_models_segmentation_path = '/mnt/DADOS_CERGY_1/bruno/save_models_parih/fine_jigsaw_grid'
    load_path = '/mnt/DADOS_CERGY_1/bruno/save_models_parih/jigsaw_grid_search'
    grid = get_grid_search_finetune_jigsaw_parih()
...
for idx in range(len(grid)):  
    ...
    name_model = f'parih_first_jigsaw_finetune_{pretext_model}_fine{idx}_1perc' 


    # Instantiating architecture.
    args['task'] = 'jigsaw'
    model_fcn = FCN(num_classes=jigsaw_classes, in_channels=args['n_channels'],task=args['task']).to(args['device'])

    #After defining a model, load 
    premodel = 'parih_first_' + pretext_model
