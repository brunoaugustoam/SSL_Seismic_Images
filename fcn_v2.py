...
class FCN(nn.Module):
    ...
    def update_task(self,new_task, new_classes):
        ...
        if self.task == None or self.task == "rotation" or self.task == "jigsaw":
            ...
            self.hog_regressor = nn.Conv2d(2048, self.new_classes, kernel_size=1, stride=1, padding=0, bias=False)


        #elif self.task == "jigsaw":
        #    #raise ValueError(f'Not yet implemented task {self.task}') 
        #    #perform concat of 9 tiles (2048*9)- must decide wether to predict the permutation idx or the 9 permutation positions
        #
        #   #or we could try using the rotation tail, just alike, adjust the names
        #    self.jigsaw_classifier =nn.Sequential(
        #        nn.Linear(2048, 512),
        #        nn.Linear(512, self.new_classes)
        #        )
    ...
    def forward(self, x):
        ...
        assert self.task== None or self.task=='segmentation' or self.task=='rotation' or self.task=='hog' or self.task=='jigsaw', "Not recognized task"
        ...
        if self.task == None or self.task == "rotation" or self.task == "jigsaw":
            ...
            inter4 = F.interpolate(activ4, size=(32,32) , mode='bilinear')
            out = self.hog_regressor(inter4)

        #elif self.task == "jigsaw":
        #    #raise ValueError(f'Not yet implemented task {self.task}')  
        #    #perform concat of 9 tiles - must decide wether to predict the permutation idx or the 9 permutation positions
        #    self.jigsaw_classifier =nn.Sequential(
        #        nn.Linear(2048, 512),
        #        nn.Linear(512, self.new_classes)
        #        )
