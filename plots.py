...
def plot_array(array_to_image):
    ...
    plt.tight_layout()
    ax = plt.subplot(1, 1, 1)
    plt.imshow(image_visible, cmap="Greys")
    plt.show()
...
def plot_pil(input_image, name='insert_number',rotation=None):
    ...
    ax = plt.subplot(1, 1, 1)
    ax.set_title(name)
    plt.imshow(input_image, cmap="Greys")
...
def plot_batch(secao, iterator, name_list, mask=None, task=None, hog_section=None,dataset='F3_netherlands'):
    ...
    if task == None or task == 'rotation' or task=='segmentation':
        ...
        ax1.imshow(image_array, cmap="Greys")
    ...
    if task == None or task=='segmentation':
        ...
        cmap_teste = LinearSegmentedColormap.from_list("mycmap", colors)
        m = plt.imshow(mask_array, alpha=0.7, cmap=cmap_teste,vmin=0,vmax=9)
        plt.colorbar(m)
...
def plot_segmentation_results(images,mask,y_hat,i,name,dataset, save=False, path=None, name_model=None):
    ...
    ax[0].imshow(section[i], cmap="Greys")
    ax[0].set_yticks([])
    ax[0].set_xticks([])
...
def plot_segy_label_dataloader(seismic_image, number, label_mask=None,plot_type='over',iterator=0,title='Title', task='segmentation',hog_section=None):
    ...
    seis = ax.imshow(image_array, cmap="Greys")
    

    colors = ['steelblue','darkturquoise', 'paleturquoise', 'mediumseagreen','lavender','wheat']
    cmap_seismic = LinearSegmentedColormap.from_list("mycmap", colors)
    ...
    if plot_type == 'img':
        ...
        lab = plt.imshow(mask_array,cmap=cmap_seismic,vmin=0,vmax=len(colors), alpha=0.3)
        fig.colorbar(lab, ax=ax)
        
    elif plot_type == 'both':
        #plt.colorbar(seis, ax=ax)
        if task == 'segmentation':
            lab = plt.imshow(mask_array,cmap=cmap_seismic,vmin=0,vmax=len(colors))
            fig.colorbar(lab, ax=ax1)
        elif task == 'hog':
...
def plot_jigsaw_loader(labels,tiles,sec_name,iterator,dataset='F3_netherlands'):
    ...
    for i, ax in zip(range(tiles.shape[1]), axs.ravel()):
            ax.imshow(tiles[iterator][i], cmap="Greys")
