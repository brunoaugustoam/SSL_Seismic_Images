...
class SeismicDataset(data.Dataset):
    def __init__(self, root, dataset_name, split,train_type='sup_ssl', task=None, n_few_shot=None, presentation_sec_type=None,cross_val=False, fold=None):
        self.root = root       
        ...
        assert self.task== None or self.task=='segmentation' or self.task=='rotation' or self.task=='hog' or self.task=='jigsaw' or self.task =='presentation', f"Not recognized task{self.task}"
            ...
            if self.cross_val == True:
                ...
                if self.split=='train':
                    if self.train_type == 'sup_ssl':
                        ...
                        inline = inline[:int(self.n_few_shot/2)]
                        crossline = crossline[:int(self.n_few_shot/2)]
                    else:
        ...
        if self.dataset_name=='Parihaka_NZPM':
            ...
            if self.task == None or self.task=='segmentation':
                ...
                if self.split=='train' or self.split == None:
                    ...
                    #add random noise
                    seg_section = seg_section + np.random.normal(loc=np.mean(seg_section),scale=np.std(seg_section))
                ...
                if self.split=='train' or self.split == None:
                    rotated_section = rotated_section + np.random.normal(loc=np.mean(rotated_section),scale=np.std(rotated_section))
                    p = np.random.randint(0,2, size=1)
                    if p == 1:
                        rotated_section = np.fliplr(rotated_section)
                    rotated_section = random_crop(rotated_section,final_size=(896,448))   

                elif self.split=='test' or self.split == 'val':
                    rotated_section = center_crop(rotated_section, final_height=896,final_width=448)
                ...
                if self.split=='train' or self.split == None:
                    resized_section = random_crop(section,final_size=(896,448))
                    resized_section = resized_section + np.random.normal(loc=np.mean(resized_section),scale=np.std(resized_section))
                    p = np.random.randint(0,2, size=1)
                    if p == 1:
                        resized_section = np.fliplr(resized_section)

                elif self.split=='test' or self.split == 'val':
                    resized_section = center_crop(section, final_height=896,final_width=448)
                else:
                    raise ValueError(f'Not recognized split {self.split}')
                ...
                labels, tiles, labels_orig, tiles_orig,permutation_idx = create_jigsaw(resized_section, self.possible_permutations, size=(256,128),dataset=self.dataset_name,split=self.split)
        ...
        ###################################################################################################################3
            ...
            if self.task == None or self.task=='segmentation':
                ...
                if self.split=='train'or self.split == None:
                    ...
                    #add random noise
                    resized_section = resized_section + np.random.normal(loc=np.mean(resized_section),scale=np.std(resized_section)) #########2
                    #add random flip
                    p = np.random.randint(0,3, size=1) #3
                    if p == 1:
                        resized_section = np.fliplr(resized_section)
                    elif p == 2:
                        resized_section = np.flipud(resized_section)
                ...
                labels, tiles, labels_orig, tiles_orig,permutation_idx = create_jigsaw(resized_section, self.possible_permutations, size=(128,192),dataset=self.dataset_name,split=self.split)    
