...
class SeismicDataset(data.Dataset):
    def __init__(self, root, dataset_name, split,train_type='sup_ssl', task=None, n_few_shot=None,cross_val=False, fold=None,present_sec='inline'):
        ...
        self.partition = [0.60,0.20,0.20]
        assert sum(self.partition) == 1  
        ...
        assert self.task== None or self.task=='segmentation' or self.task=='rotation' or self.task=='jigsaw' or self.task =='presentation' , f"Not recognized task{self.task}"
        ...
        ###############################################################################################################
            ...
            if self.task == None or self.task=='segmentation':
                ...
                if self.split=='train'or self.split == None:
                    ...
                    resized_section = center_crop(cropped_section, final_height=420,final_width=618) 
