...
class SeismicDataset(data.Dataset):
    def __init__(self, root, dataset_name, split,train_type='sup_ssl', task=None, n_few_shot=None,cross_val=False, fold=None):
        self.root = root       
        ...
        assert self.task== None or self.task=='segmentation' or self.task=='rotation' or self.task=='jigsaw' or self.task=='frame_order', f"Not recognized task{self.task}"
    ...
    def make_dataset(self):
        ...
        if self.dataset_name=='F3_netherlands':
            ...
            if self.task=='frame_order':
                filtered_files = sorted([ c for c in filtered_files if  c.split('_')[0] == 'crossline' ]) 
        ...
        #### New Zealand Parihaka  ###### SPECIFIC TRANSFORMATIONS FOR EACH DATASET ########
            ...
            if self.cross_val == True:
                ...
                if self.task == 'frame_order':
                    sections = np.hstack(np.array([inline]))
                    sec_name = inline_name
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name=='Parihaka_NZPM':
            ...
            if self.split == 'train':
                section = section + np.random.normal(loc=0,scale=0.4) #gaussian noise
                section = normalize_1(section)
            ...
            if self.task == None or self.task=='segmentation':
            ...
            elif self.task=='frame_order':
                raw_order = int(sec_name.split('_')[-1])
                classes_limit = np.arange(65,782,130) 
                classes_limit_list = list(classes_limit)
                label_order = np.array(classes_limit_list.index(min(classes_limit, key=lambda x:abs(x-raw_order))))
                ...
                return torch.from_numpy(resized_section), torch.from_numpy(label_order), raw_order, sec_name
        ...
        ###################################################################################################################3
            ...
            if self.task == None or self.task=='segmentation':
                ...
                raw_order = int(sec_name.split('.')[0].split('_')[-1])
                assert idx == raw_order, f'raw_order and index must be the same, but got {raw_order} and {idx}'
                classes_limit = np.arange(382, 1240, 156) #values selected to keep equidistance between 6 classes
                classes_limit_list = list(classes_limit)
                label_order = np.array(classes_limit_list.index(min(classes_limit, key=lambda x:abs(x-raw_order))))
